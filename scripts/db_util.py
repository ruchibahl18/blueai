...
class PropositionDatabase:
    ...
    def __init__(self):
        self.db_path = os.path.abspath(
            os.path.join(os.getcwd(), DB_DIR, USER_DB))
            ...
            with self._get_connection() as conn:
                ...
                if not propositions:
                    raise UserNotFoundError(
                        "No propositions found in the database.")
...
def fetchTopologies():
    topologiesPath = os.path.abspath(
        os.path.join(os.getcwd(), DB_DIR, 'topologies_desc.csv'))
...
def savePropositionResults(userId, bank, city, productType, subcount1,
                           subcount2, subcount3, productName, revenue,
                           moneyNeeds, customerExpNeeds, sustainabilityNeeds,
                           matchingTopologies, predictedSubscriberTakeOut):
    dbPath = os.path.abspath(os.path.join(os.getcwd(), DB_DIR, USER_DB))
    ...
    try:
        with sqlite3.connect(dbPath) as conn:
            ...
            """, (userId, bank, city, productType, subcount1, subcount2,
                  subcount3, productName, revenue, moneyNeeds,
                  customerExpNeeds, sustainabilityNeeds, matchingTopologies,
                  predictedSubscriberTakeOut))
            conn.commit()
            ...
            output = {
                'key': [
                    'Bank', 'City', 'Product Type',
                    'Subscriber count for Year 1',
                    'Subscriber count for Year 2',
                    'Subscriber count for Year 3', 'Product Name',
                    'Revenue per subscriber', ' Money Needs', 'Customer Needs',
                    'Sustainability Needs', 'Topologies you are targeting',
                    'Predicted Subscriber Count', 'Total Revenue'
                ],
                'value': [
                    bank, city, productType, subcount1, subcount2, subcount3,
                    productName, revenue, moneyNeeds, customerExpNeeds,
                    sustainabilityNeeds, matchingTopologies,
                    predictedSubscriberTakeOut,
                    (int(predictedSubscriberTakeOut) * int(revenue))
                ]
            }

            print(output)
            df = pd.DataFrame(output)
            reportsPath = os.path.abspath(
                os.path.join(os.getcwd(), REPORTS_DIR,
                             'Proposition_{}.csv'.format(propositionId)))
